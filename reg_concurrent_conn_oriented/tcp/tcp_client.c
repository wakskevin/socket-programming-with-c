...
// function prototypes
void close_up(struct addrinfo *server, int sockfd);
    ...
    int main()
{
    ...
    r = getaddrinfo("127.0.0.1", "8082", &hints, &server);
    ...
    if (r == -1)
    {
        ...
        close_up(server, sockfd); // free allocated address memory, close socket and exit
    }
    else
    {
    ...
    /* ******************* free allocated address memory and close socket ********************* */
